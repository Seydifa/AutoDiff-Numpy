...
def main():
    ...
    print(
        "  Architecture: 64 → [Dense(64, ReLU)] → [Dense(32, ReLU)] → Dense(10, Softmax)"
    )
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, num_samples, batch_size):
            ...
            # Capture graph at the first batch of epoch 0 (clean forward graph,
            # before backward/optimizer destroy the readable session state).
            if epoch == 0 and i == 0:
                out_path = (
                ...
                session.show_graphe(
                    title="Digits Classification Graph",
                    save=True,
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            print(
                f"  Epoch {epoch + 1:2d}/{epochs} | Train Loss: {avg_loss:.4f} | Test Acc: {test_acc * 100:.1f}%"
            )
